//! Wall-clock and CPU time accounting for the passes of a link.
...
impl Timers {
    ...
    /// Skips clock reads, system calls and shared recording when
    /// -print_statistics is off.
    ...
    /// Prints the timer tree to stderr, where ld64 reports its
    ...
    pub fn print(&self) {
        ...
        eprintln!("     User   System     Real  Name");
        for i in 0..records.len() {
            if records[i].parent.is_none() {
